...
class HrEmployeePrivate(models.Model):
    ...
    # private partner
    code = fields.Char(string="Code", groups="hr.group_hr_user", tracking=True)
    time_keeping_code = fields.Char(
        string="Mã chấm công", groups="hr.group_hr_user", tracking=True
    )
    workingday = fields.Date(
        string="Ngày vào làm", groups="hr.group_hr_user", tracking=True
    )
    current_residence = fields.Date(
        "Current residence", groups="hr.group_hr_user", tracking=True
    )
    tax_id = fields.Integer(string="Mã số thuế cá nhân")
    date_of_joining_the_group = fields.Date(string="Ngày vào đoàn")
    union_day = fields.Date(string="Ngày tham gia công đoàn")
    part_time_company_id = fields.Many2one("res.company", "Công ty kiêm nhiệm")
    part_time_department_id = fields.Many2one(
        "hr.department", string="Phòng ban kiêm nhiệm"
    )
    part_time_job_title = fields.Char("Chức vụ kiêm nhiệm", store=True, readonly=False)
    severance_day = fields.Date(string="Ngày thôi việc")
    resource_calendar_type = fields.Selection(
        [("1", "Cố định"), ("2", "Linh động")],
        groups="hr.group_hr_user",
        tracking=True,
        default="1",
        string="Loại giờ làm việc",
    )
    religion_id = fields.Many2one("religion", string="Tôn giáo")
    nation_id = fields.Many2one("nation", string="Dân tộc")
    issued_by_identification = fields.Many2one(
        "res.city",
    ...
    ward_id = fields.Many2one(
        "res.ward", string="Xã", domain="[('district_id', '=', district_id)]"
    )
    city_id = fields.Many2one(
        "res.city",
        string="Quê quán (Tỉnh)",
        ondelete="restrict",
        domain="[('country_id', '=', country_id)]",
    )
    current_place_of_residence = fields.Char(
        string="Nơi ở hiện tại", groups="hr.group_hr_user", tracking=True
    )
    highest_degree = fields.Char(
        string="Chứng chỉ", groups="hr.group_hr_user", tracking=True
    )
    annual_leave_fund = fields.Float(
        string="Quỹ phép năm", groups="hr.group_hr_user", tracking=True
    )
    is_check = fields.Boolean("Check quyền", default=True, compute="_get_check_type")
    car_registration = fields.Selection(
        [("YES", "Yes"), ("NO", "No")],
        groups="hr.group_hr_user",
        tracking=True,
        string="Đăng ký gửi xe",
    )
    license_plates = fields.Char(
        string="Biển số xe", groups="hr.group_hr_user", tracking=True
    )
    range_of_vehicle = fields.Char(
        string="Loại xe", groups="hr.group_hr_user", tracking=True
    )
    car_color = fields.Char(string="Màu xe", groups="hr.group_hr_user", tracking=True)
    hr_employee_relative_ids = fields.One2many(
        "hr.employee.relative", "employee_id", string="Thân nhân", copy=True
    )
    ...
    bank_account_number = fields.Char(string="STK", groups="hr.group_hr_user")
    bank = fields.Char(string="Ngân hàng", groups="hr.group_hr_user")
    bank_branch = fields.Char(string="Chi nhánh", groups="hr.group_hr_user")
    ...
    study_field = fields.Char("Chuyên ngành", groups="hr.group_hr_user", tracking=True)
    study_school = fields.Char(
        "Trường đào tạo", groups="hr.group_hr_user", tracking=True
    )
    ...
    employees_schedule_inherit_id = fields.Many2one(
        "employees_schedule_inherit", string="Employee Schedule Inherit"
    )
    ...
    employees_scheduling_ids = fields.Many2many(
        ...
        "employee_ids",
        string="Danh sách cả của nhân viên theo tuần",
    )
            ...
            if not avatar:
                if employee.user_id:
                    ...
                    avatar = base64.b64encode(
                        employee._avatar_get_placeholder())
    ...
    def _read(self, fields):
        ...
        for fname in fields:
            ...
            if self._fields[fname].translate:
                values = [(value.copy() if value else None)
                          for value in values]
    ...
    def create(self, vals_list):
        query = """
                            SELECT replace('APG'||to_char(CURRENT_DATE,'YYMMDD')||
                            TO_CHAR( COALESCE (MAX (b.seq), 0)+1, '000' ),' ','')
                  FROM (SELECT cast (SUBSTRING (a.CODE,LENGTH(a.CODE)-2,
                                           LENGTH(a.CODE)) as int) as seq
                          FROM hr_employee a where to_char(a.write_date,'YYYYMMDD')=to_char(CURRENT_DATE,'YYYYMMDD') 
                          and a.code <>''  )b
                        """
        self._cr.execute(query)
        max_code = self._cr.fetchall()
        code_employee = max_code[0][0]

        for vals in vals_list:
            if not hasattr(vals, 'code'):
                vals['code'] = code_employee
            elif vals['code'] == '':
                vals['code'] = code_employee
            if vals.get('user_id'):
                user = self.env['res.users'].browse(vals['user_id'])
                vals.update(self._sync_user(
                    user, bool(vals.get('image_1920'))))
                vals['name'] = vals.get('name', user.name)

                # vals_list[0].append({'code': 'NV123'})
                # vals.append({'code': 'NV1234'})
                # vals_list = vals
    ...
    @api.depends("resource_calendar_type")
    def _get_check_type(self):
        if self.resource_calendar_type == "1":
            self.is_check = False
        else:
            self.is_check = True
    ...
    #         employee.part_time_job_title = employee.job_id.name

    # def _code_employee(self):
    #     query = """
    #                 SELECT replace('APG'||to_char(CURRENT_DATE,'YYYYMMDD')||
    #                 TO_CHAR( COALESCE (MAX (b.seq), 0)+1, '0000' ),' ','')
    #       FROM (SELECT cast (SUBSTRING (a.CODE,LENGTH(a.CODE)-3,
    #                                LENGTH(a.CODE)) as int) as seq
    #               FROM hr_employee a where to_char(a.create_date,'YYYYMMDD')=to_char(CURRENT_DATE,'YYYYMMDD')  )b
    #             """
    #     self._cr.execute(query)
    #     max_code = self._cr.fetchall()
    #     code_employee = max_code[0][0]
