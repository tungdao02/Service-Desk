...
class SlideQuestion(models.Model):
    ...
    slide_id = fields.Many2one(
        'slide.slide', string="Content",  ondelete='cascade',required=True)
    slide_partner_id = fields.Many2one(
        'slide.slide.partner', string="Content Partner", ondelete='cascade') #, required=True
    exam_id = fields.Many2one(
        'slide.exam', string="Content", ondelete='cascade')#, required=True
    score = fields.Float("Score")
    answer_ids = fields.One2many(
        'slide.answer', 'question_id', string="Answer", copy=True)
    category_id = fields.Many2one('slide.categories', string="Category")
    # statistics
    attempts_count = fields.Integer(
        compute='_compute_statistics', groups='website_slides.group_website_slides_officer')
    attempts_avg = fields.Float(compute="_compute_statistics", digits=(
        6, 2), groups='website_slides.group_website_slides_officer')
    ...
    def _compute_statistics(self):
        ...
        slide_stats = dict((s.slide_id.id, dict(
            {'attempts_count': 0, 'attempts_unique': 0, 'done_count': 0})) for s in slide_partners)
        ...
        for question in self:
            ...
            question.attempts_count = stats.get(
                'attempts_count', 0) if stats else 0
            question.attempts_avg = stats.get(
                'attempts_count', 0) / stats.get('attempts_unique', 1) if stats else 0
