...
class SlidePartnerRelation(models.Model):
    ...
    question_ids = fields.Many2many(
        "slide.question", string="Questions", copy=True)
    ...
    def create(self, vals_list):
        ...
        print('-------------------------', vals_list)
        if vals_list != []:
            slide_id = vals_list[0]['slide_id']
            slide = self.env['slide.slide'].browse(slide_id)
            print(slide)
            # questions = self.env['slide.question'].search([])
            questions = self.env['slide.question'].search([('category_id', '=', slide.question_category_id.id)])
            print('----------------------\n', questions, '\n-------------------------')
            if slide.type_quizz == 'randomcategory':
                count = slide.random_question_count
                if len(questions) < count:
                    raise ValidationError("Not enough questions available.")
                random_questions = sample(questions, count)
                print('----------------------\n', random_questions, '\n-------------------------')
                vals_list[0]['question_ids'] = [(6, 0, [q.id for q in random_questions])]
        # # end customer
...
class Slide(models.Model):
    ...
    image_1920 = fields.Image(compute="_compute_image_1920",
                              store=True, readonly=False)  # image.mixin override
    ...
    is_new_slide = fields.Boolean(
        'Is New Slide', compute='_compute_is_new_slide')
    ...
    questions_count_exam = fields.Integer(
        string="Numbers of Questions")
    question_category_id = fields.Many2one('slide.categories', string="Category")
    # Start Customer
    type_quizz = fields.Selection([('choose','Choose'),('randomcategory','Random Category')],string='Exam Type',default='choose')
    random_question_count = fields.Integer('Numbers of questions')
    ...
    quiz_first_attempt_reward = fields.Integer(
        "Reward: first attempt", default=10)
    quiz_second_attempt_reward = fields.Integer(
        "Reward: second attempt", default=7)
    quiz_third_attempt_reward = fields.Integer(
        "Reward: third attempt", default=5,)
    ...
    # content - webpage
    html_content = fields.Html("HTML Content", help="Custom HTML content for slides of category 'Article'.",
                               translate=True, sanitize_attributes=False, sanitize_form=False)
    # content - images
    ...
    website_id = fields.Many2one(
        related='channel_id.website_id', readonly=True)
    ...
    embed_code = fields.Html('Embed Code', readonly=True,
    ...
    total_slides = fields.Integer(
        compute='_compute_slides_statistics', store=True)
    ...
    def _compute_slides_statistics(self):
        ...
        keys = ['nbr_%s' % slide_category for slide_category in self.env['slide.slide']
                ._fields['slide_category'].get_values(self.env)]
        default_vals = dict((key, 0) for key in keys + ['total_slides'])

        res = self.env['slide.slide']._read_group(
            [('is_published', '=', True), ('category_id',
                                           'in', self.ids), ('is_category', '=', False)],
    ...
    def _compute_slides_statistics_category(self, read_group_res):
        ...
        slide_categories = self.env['slide.slide']._fields['slide_category'].get_values(
            self.env)
        keys = ['nbr_%s' % slide_category for slide_category in slide_categories]
        result = dict((cid, dict((key, 0)
                      for key in keys + ['total_slides'])) for cid in self.ids)
        ...
        for slide in self:
            ...
            if slide.slide_category == 'video':
                if slide.video_source_type == 'youtube':
                    ...
                    embed_code = Markup(
                        '<iframe src="//www.youtube-nocookie.com/embed/%s?%s" allowFullScreen="true" frameborder="0"></iframe>') % (slide.youtube_id, query_params)
                elif slide.video_source_type == 'google_drive':
                    embed_code = Markup(
                        '<iframe src="//drive.google.com/file/d/%s/preview" allowFullScreen="true" frameborder="0"></iframe>') % (slide.google_drive_id)
                ...
                slide_url = base_url + \
                    url_for('/slides/embed/%s?page=1' % slide.id)
                slide_url_external = base_url + \
                    url_for('/slides/embed_external/%s?page=1' % slide.id)
    ...
    def _compute_video_source_type(self):
        for slide in self:
            ...
            youtube_match = re.match(
                self.YOUTUBE_VIDEO_ID_REGEX, slide.video_url) if slide.video_url else False
            ...
            vimeo_match = re.search(
                self.VIMEO_VIDEO_ID_REGEX, slide.video_url) if slide.video_url else False
            ...
            if slide.video_url and slide.video_source_type == 'vimeo':
                ...
                if match and len(match.groups()) == 3:
                    if match.group(3):
                        ...
                        slide.vimeo_id = '%s/%s' % (match.group(2),
                                                    match.group(3))
                    else:
    ...
    def _compute_website_url(self):
        ...
        for slide in self:
            if slide.id:  # avoid to perform a slug on a not yet saved record in case of an onchange.
                ...
                slide.website_url = '%s/slides/slide/%s' % (
                    base_url, slug(slide))
    ...
    def create(self, vals_list):
        ...
        can_publish_channel_ids = self.env['slide.channel'].browse(
            channel_ids).filtered(lambda c: c.can_publish).ids
            ...
            if any(vals.get(url_param) for url_param in ['url', 'video_url', 'document_google_url', 'image_google_url']) \
               ...
               and not self.env.context.get('website_slides_skip_fetch_metadata'):
                ...
                if slide_metadata:
                    ...
                    slide.update(
                        {key: value for key, value in slide_metadata.items() if key not in vals.keys()})
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        groups = super(Slide, self)._notify_get_recipients_groups(
            msg_vals=msg_vals)
        if not self:
            return groups
    ...
    def _post_publication(self):
        for slide in self.filtered(lambda slide: slide.website_published and slide.channel_id.publish_template_id):
            ...
            html_body = publish_template.with_context(
                base_url=slide.get_base_url())._render_field('body_html', slide.ids)[slide.id]
            subject = publish_template._render_field(
                'subject', slide.ids)[slide.id]
            ...
            kwargs = {}
            reply_to = publish_template._render_field(
                'reply_to', slide.ids)[slide.id]
            if reply_to:
    ...
    def _send_share_email(self, email, fullscreen):
        ...
        for record in self:
            ...
            mail_ids.append(template.send_mail(
                record.id, email_layout_xmlid='mail.mail_notification_light', email_values=email_values))
        return mail_ids
    ...
    def _action_mark_completed(self):
        uncompleted_slides = self.filtered(
            lambda slide: not slide.user_has_completed)
    ...
    def action_mark_uncompleted(self):
        ...
        completed_slides = self.filtered(
            lambda slide: slide.user_has_completed)
    ...
    def _action_set_quiz_done(self, completed=True):
        ...
        for slide in self:
            ...
            points += gains[user_membership_sudo.quiz_attempts_count -
                            1] if user_membership_sudo.quiz_attempts_count <= len(gains) else gains[-1]
    ...
    def _compute_quiz_info(self, target_partner, quiz_done=False):
        ...
        slide_partners_map = dict((sp.slide_id.id, sp)
        ...
        for slide in self:
            ...
            result[slide.id] = {
                ...
                # what would be gained at next test
            ...
            if slide.question_ids and slide_partner and slide_partner.quiz_attempts_count:
                result[slide.id]['quiz_karma_gain'] = gains[slide_partner.quiz_attempts_count] if slide_partner.quiz_attempts_count < len(
                    gains) else gains[-1]
                result[slide.id]['quiz_attempts_count'] = slide_partner.quiz_attempts_count
                if quiz_done or slide_partner.completed:
                    result[slide.id]['quiz_karma_won'] = gains[slide_partner.quiz_attempts_count -
                                                               1] if slide_partner.quiz_attempts_count < len(gains) else gains[-1]
        return result
    ...
    def _fetch_external_metadata(self, image_url_only=False):
        ...
        if self.slide_category == 'video' and self.video_source_type == 'youtube':
            slide_metadata, error = self._fetch_youtube_metadata(
                image_url_only)
        elif self.slide_category == 'video' and self.video_source_type == 'google_drive':
            slide_metadata, error = self._fetch_google_drive_metadata(
                image_url_only)
    ...
    def _fetch_youtube_metadata(self, image_url_only=False):
        ...
        self.ensure_one()
        google_app_key = self.env['website'].get_current_website(
        ...
        if error_message:
            _logger.warning(
                'Could not fetch YouTube metadata: %s', error_message)
            return {}, error_message

        slide_metadata = {'slide_type': 'youtube_video'}
        youtube_values = response.get('items')[0]
        youtube_duration = youtube_values.get(
    ...
    def _fetch_google_drive_metadata(self, image_url_only=False):
        ...
        if error_message:
            _logger.warning(
                'Could not fetch Google Drive metadata: %s', error_message)
            return {}, error_message
        ...
        if google_drive_values.get('thumbnailLink'):
            ...
            thumbnail_url = google_drive_values['thumbnailLink'].replace(
                '=s220', '')
            if image_url_only:
                slide_metadata['image_url'] = thumbnail_url
            else:
        ...
        if self.slide_category == 'document':
            ...
            completion_time = float(
                google_drive_values.get(
                    'videoMediaMetadata', {}).get('durationMillis', 0)
    ...
    def _fetch_vimeo_metadata(self, image_url_only=False):
        ...
        try:
            response = requests.get(
                'https://vimeo.com/api/oembed.json?%s' % urls.url_encode(
                    {'url': self.video_url}),
                timeout=3
            )
            response.raise_for_status()
        ...
        if error_message:
            _logger.warning(
                'Could not fetch Vimeo metadata: %s', error_message)
            return {}, error_message
        ...
        if vimeo_values.get('duration'):
            ...
            slide_metadata['completion_time'] = vimeo_values.get(
                'duration') / (60 * 60)
    ...
    def _get_completion_time_pdf(self, data_bytes):
        ...
        if data_bytes.startswith(b'%PDF-'):
            try:
                pdf = PyPDF2.PdfFileReader(io.BytesIO(
                    data_bytes), overwriteWarnings=False)
    ...
    def _search_get_detail(self, website, order, options):
        ...
        if with_description:
            ...
            mapping['description'] = {
                'name': 'description', 'type': 'text', 'html': True, 'match': True}
        return {
            'model': 'slide.slide',
    ...
    def _search_render_results(self, fetch_fields, mapping, icon, limit):
        ...
        for slide, data in zip(self, results_data):
            data['_fa'] = icon_per_category.get(
                slide.slide_category, 'fa-file-pdf-o')
            data['url'] = slide.website_url
            data['course'] = _('Course: %s', slide.channel_id.name)
            data['course_url'] = slide.channel_id.website_url
        return results_data

    # @api.onchange('type_choose_question')
    # def _random_question(seft):
    #     for slice in seft:
    #         if slice.type_choose_question == 'random_categories':
    #             question_category = seft.env['slide.question'].search([('category_id', '=', slice.question_category_id.name)])
    #             print(question_category)
    #             slice.question_ids =question_category
