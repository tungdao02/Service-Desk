...
class WebsiteSlides(WebsiteProfile):
    ...
    def _slide_quiz_random_data(self,slide):
        is_designer = request.env.user.has_group('website.group_website_designer')
        # Lấy thông tin res.partner của người dùng đang đăng nhập
        user_partner = request.env.user.partner_id
        print('------------------------',slide)
        print('-------------------', user_partner)

        # Lấy tất cả các bản ghi slide.partner tương ứng với slide.slide
        slide_partner_records = request.env['slide.slide.partner'].search([
            ('partner_id', '=', user_partner.id),('slide_id','=',slide.id)
        ])
        print('-------------------', slide_partner_records)
        # Lấy các bản ghi slide.slide tương ứng với slide_partner_records
        # slide_records = slide_partner_records.mapped('slide_id')
        # print('-------------------', slide_records)
        # Lấy danh sách các question_ids trong slide_records
        question_ids = slide_partner_records.mapped('question_ids')
        print('-------------------',question_ids,'---------------------')
        values = {
            'slide_questions': [{
                'id': question.id,
    ...
    def slide_channel_join(self, channel_id):
        ...
        channel = request.env['slide.channel'].search([
            ('id', '=', channel_id)])
        channel_define = request.env['slide.channel'].search([
            ('name', '=', '000000000000000')])

        parent_channel = request.env['slide.channel'].search([
            ('id', '=', channel.parent_channel_id.id)])

        parent_channel_partner = request.env['slide.channel.partner'].search([
            ('partner_id', '=', user_partner.id), ('channel_id', '=', parent_channel.id)
        ])
        if parent_channel_partner.completed == True or channel.parent_channel_id == channel_define:
            if request.website.is_public_user():
            ...
            erorr = 'Bạn chưa hoàn thành khóa học ' + parent_channel.name
            raise UserError(erorr)
            return {'error': 'Bạn chưa hoàn thành khóa học trước'}
    ...
    def slide_quiz_get(self, slide_id):
        ...
        if slide.type_quizz == 'randomcategory':
            result = self._slide_quiz_random_data(slide)
        return result
    ...
    def slide_quiz_submit(self, slide_id, answer_ids):
        ...
        if slide.type_quizz == 'randomcategory':
            ...
            all_questions = slide_partner_records.mapped('question_ids')
            ...
            if not user_bad_answers:
                ...
                rank_progress.update({
                    ...
                    'level_up': rank_progress['previous_rank']['lower_bound'] != rank_progress['new_rank'][
                        'lower_bound']
                })
            self._channel_remove_session_answers(slide.channel_id, slide)
        else :
