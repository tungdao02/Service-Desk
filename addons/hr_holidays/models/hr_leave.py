...
class HolidaysRequest(models.Model):
    ...
    browsing_object = fields.Selection([('NQL', 'Người quản lý'), ('NHD', 'Người hướng dẫn')], string='Đối tượng duyệt',
                                       default='NQL')
    approved_by_id = fields.Many2one('hr.employee', compute='_compute_approved_by',
                                     default=lambda self: self.env.user.parent_id)
    ...
    employee_job_title = fields.Char(related='employee_id.job_title', readonly=True, store=False, string="Chức vụ")
    employee_parent_id = fields.Many2one(related='employee_id.parent_id', readonly=True, store=False, string="Chịu trách nhiệm")
    employee_coach_id = fields.Many2one(related='employee_id.coach_id', readonly=True, store=False)
    ...
    def _compute_approved_by(self):
        for claim in self:
            ...
            if manager == 'NQL':
                claim.approved_by_id = claim.employee_parent_id
            elif manager == 'NHD':
                claim.approved_by_id = claim.employee_coach_id
            else:
