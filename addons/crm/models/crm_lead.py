...
from odoo.addons.crm.wizard import crm_phone_utils
...
class Lead(models.Model):
    ...
    _order = "phone_call_ref_ids desc"
    ...
    phone_call_ref_ids = fields.One2many('crm.phone.call.trans', 'phone_call_ref_id', string='Phone Call')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            vals['phone'] = str(vals['phone']).replace(' ', '')
    ...
    @api.model
    def call_end_handler(self, data):
        data = data['data']
        record_file = crm_phone_utils.get_trans_record_file(self, data['uuid'])
        values = {'phone': data['phone'], 'transaction_id': data['uuid'], 'sipUser': data['sipUser'],
                  'totalDuration': data['duration'], 'ringDuration': data['ringing'], 'sipNumber': data['sipNumber'],
                  'phone_call_ref_id': data['lead_id'], 'record_file': record_file,
                  'user_id': self.env.user.id}

        obj_id = self.env['crm.phone.call.trans'].create(values).id

        return obj_id

    @api.model
    def get_phone_info(self, data):
        phones = self.env['res.users'].search([('id', '=', self.env.user.id)]).phone_ext_numbers
        for phone in phones:
            if phone['enabled']:
                return {'domain': phone.domain, 'sip_user': phone.ext_number, 'password': phone.password}
        return None

    @api.model
    def call_note_end_handler(self, data):
        data = data['data']
        print('call_note_end_handler')
        if data['callId'] is not None:
            values = {'note': data['note1']}
            self.env['crm.phone.call.trans'].search([('transaction_id', '=', data['callId'])]).update(values)
    ...
    def start_call(self):
        ...
        #     'view_type': 'form',
        #     'view_mode': 'form',
        #     'view_id': self.env.ref('crm.crm_lead_view_omi_call').id,
