...
class Team(models.Model):
    ...
    x_assign_all = fields.Boolean(
        string='Manual assign:')
    x_assign_count = fields.Integer(
        string='')
    ...
    team_lead_ids = fields.One2many('crm.lead', 'team_id', 'Team Leads')


    def _compute_assign_all(self):
        self.x_assign_all = True
    ...
    def _cron_assign_leads(self, work_days=None):
        ...
        result = self.env['crm.team'].search([
            '&', '|', ('use_leads', '=',
                       True), ('use_opportunities', '=', True),
            ('assignment_optout', '=', False)
        ])
        result._action_assign_leads(work_days=work_days)
        return True
    ...
    def action_assign_leads(self, work_days=1, log=True):
        ...
        :return action: a client notification giving some insights on assign
          process;
        """
        # team_count = len(self.crm_team_member_ids)
        # if self.x_assign_all and team_count != 0:
        #     work_days = self.x_assign_count / team_count
        #
        ...
        values = {'team_id': self.id }
        rule_id = self.env['crm.team.assign.rule'].create(values).id
        return {
            'name': 'Team: '+ self.name,
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': self.env.ref('crm.crm_team_assign_form').id,
            'res_model': 'crm.team.assign.rule',
            'res_id': rule_id,
    ...
    def _action_assign_leads(self, work_days=1):
        ...
        members_data = self.crm_team_member_ids._assign_and_convert_leads(self.id, work_days=work_days)
